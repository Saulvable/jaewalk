...
export function renderSummary(tripName, points) {
  ...
  if (!panel) return
  panel.innerHTML = `
    <div style="display:flex;align-items:center;gap:10px;font-size:11px;color:#777;flex-wrap:wrap;line-height:1.4">
      <span style="font-weight:700;color:#FF3D5A;font-size:12px">${tripName}</span>
      <span>${days}일차</span>
      <span>${points.length}개 장소</span>
      <span style="color:#F39C12">총이동비용 $${totalCost.toFixed(0)}</span>
    </div>`
}
