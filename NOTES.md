...
- ✅ pdf 이거 여전히 안되... PDF 생성 실패: too many function arguments   .. 이렇게 나와. → btoa chunk 방식으로 수정 (main.js loadNanumGothic)
- ✅ 메뉴가 여전히 커. 포인트 위로가기 아래로가기 복사하기 아이콘있잖아 사이드바에. 그걸 세로로배치하지말고 가로로배치하면 더 여백이 줄어들거 같은데.. 그렇게 해줘.
- 그리고 한곳에 여러 포인트가 있을수있잖아. 숙소같은경우는 한 장소에 여러개의 포인트가 있을텐데, 그게 다 겹쳐있어서 지도로 클릭하기 힘들어. 그거 어떻게 다 보이는 방법없나? 살짝 옆으로 빠지게 할까? 포인트가 정확하지 않아도 되니깐 어차피 건물은 크잖아. -> 이거 가능해? 가능한지 안한지 알려줘. 
- 그리고 지도 수정하거나 포인트 이동하거나 할때마다 지도가 전체 경로가 보이게끔 커지네. 그거 안해도되. 새장소가 되면 새장소 부근으로 보여주고, 포인트를 마우스나 손가락으로 이동하면 이동된 부근으로해줘. ->이거 웹사이트에선 되는데 핸드폰에선 안되네 여전히 전체지도로 바뀌어 수정하면. 
...
| v0.12 | 2026-06-09 | PDF btoa chunk 수정(main.js), ▲▼⧉ 버튼 가로배치(index.html CSS), 파비콘 재제작 — 횡단보도 폭 확대 + 발자국 크기/선명도 향상(icon-512.png, icon-192.png, favicon.ico) |
| v0.13 | 2026-06-09 | 파비콘 새 발자국(bird track)으로 교체 — Y자형 3+1 발가락, 줄 2개(160px), 좌우 교차 배치. onDayFilter isInitial:true→false 수정 — 일차탭/모바일 수정 시 fitBounds 방지 |
| v0.14 | 2026-06-09 | 모바일 fitBounds 근본 수정 — map.js에 getMapView/setMapView 추가, savePoint/deletePoint/onMoveUp/onMoveDown에서 뷰 저장→복원. 새 포인트면 flyTo. 마커 겹침 분산 THRESH 5m→20m, radius 12m→25m |
